...
def make_embedding(FR_sec, method="dca1", n_components=8, random_state=None):
    """
    Compute low-dimensional embedding of firing rates and return:
      - Z : (T, d)  time-series in DR space
      - meta : dict describing method
      - V_dr : (N, d) projection matrix from neurons -> DR dims
               so that (approximately) Z ≈ X @ V_dr,
               where X is the standardized firing-rate matrix.
    """
    X = _std_nan_robust(FR_sec)   # (T, N) z-scored per neuron

    if method == "pca":
        model = PCA(n_components=n_components, random_state=random_state).fit(X)
        Z = model.transform(X)                         # (T, d)
        V_dr = model.components_.T.astype(np.float32)  # (N, d)
        meta = {
            "method": "pca",
            "n_components": int(Z.shape[1]),
        }
        return Z.astype(np.float32), meta, V_dr

    elif method in ("dca1", "dca1_sym"):
        symmetric = (method == "dca1_sym")
        # _time_lagged_projection already returns (Z, V)
        Z, V_dr = _time_lagged_projection(
            X, d=n_components, lag=1, ridge=1e-6, symmetric=symmetric
        )
        meta = {
            "method": method,
            "n_components": int(Z.shape[1]),
        ...
        return Z.astype(np.float32), meta, V_dr.astype(np.float32)
...
def fit_srnn_with_split(cfg: TrainConfig) -> Dict:
    ...
    FRz_full = np.nan_to_num((FR_sec - mu_full) / sd_full, 0.0, 0.0, 0.0)
        ...
        # ----- DR -----
    # Z_raw : (T, d_in)    embedded time-series
    # V_dr  : (N, d_in)    neuron → DR projection
    Z_raw, dr_meta, V_dr = make_embedding(
        FR_sec,
        method=cfg.dr_method,
        n_components=cfg.dr_n,
        random_state=cfg.dr_random_state,
    )

    # Standardize DR coordinates (this is what the SRNN actually sees)
    scaler_z = StandardScaler().fit(Z_raw)
    Zz = scaler_z.transform(Z_raw).astype(np.float32)   # (T, d_in)
    d_in = Zz.shape[1]

    # Store DR + scaler metadata for later interpretability
    dr_meta["z_mean"] = scaler_z.mean_.tolist()
    dr_meta["z_scale"] = scaler_z.scale_.tolist()
    dr_meta["projection_shape"] = list(V_dr.shape)  # [N, d_in]
    ...
    if H_list:
        ...
        # ----- neuron × latent "emission" weights -----
    #
    # We have:
    #   - V_dr : (N, d_in)   neuron -> DR (Z_raw) projection
    #   - scaler_z.scale_ : per-DR-dim scaling used to get Zz
    #   - W : (H, d_in)      latent h -> Zz decoder
    #
    # The linear part of the mapping is approximately:
    #   Zz ≈ X_std @ (V_dr @ diag(1 / z_scale))
    #   Zz ≈ h @ W
    #
    # So neuron → latent weights can be approximated by:
    #   E = V_dr @ diag(1 / z_scale) @ W^T     (N × H)
    # where rows = neurons, cols = latent dimensions.
    #
    try:
        if V_dr is not None and H_list:
            z_scale = np.asarray(scaler_z.scale_, float)
            z_scale[z_scale == 0.0] = 1.0
            S_inv = np.diag(1.0 / z_scale)              # (d_in, d_in)

            # V_dr : (N, d_in), W.T : (d_in, H)
            E_neuron_latent = V_dr @ S_inv @ W.T        # (N, H)

            np.save(save_dir / "emissions_neuron_by_latent.npy",
                    E_neuron_latent.astype(np.float32))
        else:
            E_neuron_latent = None
    except Exception as e:
        print(f"[warn] could not compute neuron×latent emissions: {e}")
        E_neuron_latent = None
    ...
    np.save(save_dir / "V_dr.npy", V_dr.astype(np.float32))  # neuron -> DR projection
